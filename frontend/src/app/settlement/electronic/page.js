...
function ElectronicDashboardContent() {
  ...
  const fetchTrade = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/settlement/electronic/${tradeRef}`, {
  ...
  const handleAction = async (action, editData = null) => {
    ...
    try {
      const token = sessionStorage.getItem("auth_token");
      const res = await fetch(`/api/settlement/electronic/action`, {
        method: "POST",
        headers: {
  ...
  if (loading) {
    return <div className="p-8 text-white">Loading Electronic Dashboard...</div>;
  }
    ...
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold">Electronic Settlement Dashboard</h1>
          ...
          <div className="bg-gray-800 p-6 rounded-lg border border-gray-700 shadow-xl">
            <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-700">
              ...
              <button
                ...
                title={trade.currentStatus !== "SETTLEMENT_BREAK" ? "You must Raise Break before editing" : ""}
