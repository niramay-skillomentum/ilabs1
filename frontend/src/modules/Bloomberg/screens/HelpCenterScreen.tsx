      ...
      const renderCategory = (title: string, cmds: string[]) => {
        ...
        return (
          <div style={{ marginBottom: '24px' }}>
            <div style={{ color: '#ff9900', fontWeight: 'bold', marginBottom: '8px', borderBottom: '1px solid #333', paddingBottom: '4px' }}>{title}</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {categoryCommands.map((c, idx) => (
                <div key={idx} style={{ display: 'flex', fontSize: '13px', fontFamily: 'Fira Code, monospace' }}>
                  <div style={{ color: '#fff', width: '120px', flexShrink: 0 }}>{c.cmd}</div>
                  <div style={{ color: '#888', marginRight: '8px' }}>-</div>
