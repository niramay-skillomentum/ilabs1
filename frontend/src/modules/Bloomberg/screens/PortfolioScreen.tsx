...
export default function PortfolioScreen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError('');
      try {
        // 1. Determine user's active desk session
        ...
        // 2. Fetch portfolio filtered by desk and assignment
        const res = await bloombergApi.getPortfolio({ desk, assignedOnly: true });
        if (res.success) {
  ...
  return (
    <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', height: '100%', overflowY: 'auto' }}>
      ...
      {!loading && !currentDesk && !error && (
        <div style={{ padding: '16px', color: 'var(--bb-text-secondary)' }}>
          No active desk session found. Generate a queue first to view your portfolio.
        </div>
      )}

      {!loading && currentDesk && portfolioData && (
